...
import {
  linkIssueToPointAction,
  searchIssuesForLinkingAction,
  type PointActionState,
} from "@/app/dashboard/discussions/[discussionId]/points-actions";

const initialState: PointActionState = {};

// Search-before-link UI: per the approved design, an existing Issue is
// always searched for first, to avoid creating a duplicate. This component
// is the search box; linking submits linkIssueToPointAction directly (no
// separate confirmation step needed — the Server Action re-validates the
// Issue exists).
export default function IssueSearchLink({ discussionId, pointId }: { discussionId: string; pointId: number }) {
  const [term, setTerm] = useState("");
  const [results, setResults] = useState<IssueListItem[]>([]);
  const [isPending, startTransition] = useTransition();
  const [state, formAction, linkPending] = useActionState(linkIssueToPointAction, initialState);
  ...
  function runSearch(value: string) {
    ...
    startTransition(async () => {
      const found = await searchIssuesForLinkingAction(value);
      setResults(found);
    });
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <input
        ...
        className="rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-3 py-1.5 text-xs text-neutral-900 dark:text-neutral-100 w-72"
      />
