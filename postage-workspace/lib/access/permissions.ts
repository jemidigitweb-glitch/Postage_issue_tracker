...
const ROLE_PERMISSIONS: Readonly<Record<Role, ReadonlySet<Permission>>> = {
  ...
  //
  // mobile:submit lets this role also use /mobile — the only place this
  // role's permission set differs from main's, since main has no /mobile
  // route to hold that permission for.
  raised_by: new Set<Permission>([
    "issue:view_all",
    "issue:create",
    "issue:edit",
    "issue:delete",
